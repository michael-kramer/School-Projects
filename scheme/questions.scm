...
(define (list-partitions total)
  (let ((list-partitions-atmost
    (mu (total atmost usedatmost)
      (cond ((= total 0) ' (()))
            ((< total 0) ' ())
            ((< atmost 1) ' ())
            (else (append
                  (cons-all atmost (list-partitions-atmost (- total atmost) atmost #t))
                  (list-partitions-atmost total (- atmost (if usedatmost 2 1)) #f))
            )
        )
    )
  ))
  (list-partitions-atmost total total #f)
  )
)
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (cons (append (list 'lambda (car (analyze (zip values)))) (analyze body)) (cadr (analyze (zip values)))) 
           ))
        (else
