...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("numbers of parameters and values do not match")
...
def do_if_form(expressions, env, tail=False):
    ...
    if scheme_true(scheme_eval(expressions[0], env)):
        return scheme_eval(expressions[1], env, tail)
    else:
        if len(expressions) == 3:
            return scheme_eval(expressions[2], env, tail)
        else:
            return okay
...
def do_and_form(expressions, env, tail=False):
    ...
    while expressions is not nil:
        ...
        last = expressions.second is nil
        expr_val = scheme_eval(expr, env, last and tail)
        if not scheme_true(expr_val):
            return expr_val
        else:
            value = expr_val
        expressions = expressions.second
...
def do_cond_form(expressions, env, tail=False):
    ...
    for i, clause in enumerate(expressions):
        ...
        if scheme_true(test):
            if clause.second is nil:
                return test
            return eval_all(clause.second, env, tail)
    return okay
...
def make_let_frame(bindings, env):
    ...
    for binding in bindings:
        ...
        if not scheme_symbolp(binding.first):
            raise SchemeError("attempting to bind value to something not a symbol")
        new_env.define(binding.first, eval_all(binding.second, env))
    return new_env
...
def check_formals(formals):
    ...
    so_far = set()
    while formals != nil:
        check_form(formals, 1)
        value = formals.first
        if not scheme_symbolp(value):
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Evaluate):
        ...
        first, rest = expr.first, expr.second
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            result = SPECIAL_FORMS[first](rest, env, tail)
        else:
            procedure = scheme_eval(first, env)
            args = rest.map(lambda operand: scheme_eval(operand, env))
            result = scheme_apply(procedure, args, env)
